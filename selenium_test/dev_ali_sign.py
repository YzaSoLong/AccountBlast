...
class IpsSign:
    ...
    def __init__(self):
        ...
        # 错误判断标签
        self.warn_xpath_string = global_var.ali_warn_string
        # 获取driver
        ...
        # 登录按钮、账户、密码的标签id
        self.login_button_xpath = global_var.ali_login_button_xpath

        self.user_name_xpath = global_var.ali_user_name_xpath

        self.password_id = global_var.ali_password_id
    ...
    # 获取driver
    def get_driver(self):
        self.driver.get(global_var.ali_login_url)

    # 输入账户密码
    def enter_count_and_password(self):
        driver = self.driver
        driver.find_element_by_xpath(self.user_name_xpath).clear()


        print("当前账号" + self.single_row_dict['user_name_dic'])
        driver.find_element_by_xpath(self.user_name_xpath).send_keys(self.single_row_dict['user_name_dic'])
    ...
    def loop_to_sign(self):
        ...
        try:
            element = WebDriverWait(driver, 20).until(
                EC.presence_of_element_located((By.ID, "login-form-iframe"))
            )
